...
public class LRTrain {
    public static void main(String[] args) throws IOException {
        // 初始化spark运行环境
        SparkSession spark = SparkSession.builder().master("local").appName("recommended").getOrCreate();

        // 加载特征及 label 训练文件
        JavaRDD<String> csvFile = spark.read().textFile("file:///E:/code/JavaEE/recommendedsystem/src/main/resources/feature.csv").toJavaRDD();
        ...
        // 设置正则化系数和设置多分类问题为了防止过拟合
        LogisticRegression lr = new LogisticRegression().setMaxIter(10).setRegParam(0.3).setElasticNetParam(0.8).setFamily("multinomial");

        LogisticRegressionModel logisticRegressionModel = lr.fit(trainData);

        logisticRegressionModel.save("file:///E:/code/JavaEE/recommendedsystem/src/main/resources/lrmode");

        // 测试评估
        Dataset<Row> predictions = logisticRegressionModel.transform(testData);
